...
from gi.repository import Gtk, GLib #Gst, GObject, Gtk, Gdk,
...
class Main_Window(Gtk.Window):
    ...
    def __init__(self):
        ...
        self.message_dialog_timeout("Auto-timeout Message.", 
                "Message posted at: {}. Random timeout of {} seconds."
    ...
    def setup_window(self):
        ...
        self.button_1.connect("clicked", self.button_callback)
        self.vbox.pack_start(self.button_1, expand=False, fill=False, padding=0)
        self.add(self.vbox)
        self.show_all()
        
        
    def button_callback(self, button_widget):
            """
            Launch message dialog for a random amount of time.
            Message dialog is automaticall destroyed
            """
            time_out = random.randint(1, 5)
            time_string = datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")
            # Launch Gtk.MessageDialog()
